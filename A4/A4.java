...
public class A4  extends JFrame {
	...
	class AddActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			JFrame f = new JFrame();
			if (node == null) {
				JOptionPane.showMessageDialog(f, "ERROR: No node selected.");
				return;
			} 
			if (!(node instanceof NestedShape)) {
				JOptionPane.showMessageDialog(f, "ERROR: Must select a NestedShape node.");
				return;
			}
	...
	class RemoveActionListener implements ActionListener {
		public void actionPerformed(ActionEvent e) {
			...
			if (node == panel.getRoot()) {
				JOptionPane.showMessageDialog(f, "ERROR: Must not remove the root.");
				return;
			}
